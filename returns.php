...
<html lang="en">
  <head>
    ...
    <link href="css/css_return.css" rel="stylesheet" type="text/css">
...
<!-- section -->
<section id="returnsdetail" name="returnsdetail">
...
<table class="table table-striped table-bordered table-hover table-dark">
  ...
  <tbody>
            ...
            <?php
            $bid=$_POST["returnselect"];
            $today=date('Y-m-d');
            $today=date('Y-m-d',strtotime($today));
            
            

            $sql="Insert into returns (bid,uid,returndate) values(".$bid.",".$userid.",'".$today."');";
                ...
                else 
                {
                    ...
                    if (!mysqli_query($conn, $sql)) 
                    {
                        ...
                        $sql = "UPDATE borrow SET returned= 1 WHERE bid = ".$bid." and uid = ".$userid. " and returned= 0;";
                    if (!mysqli_query($conn, $sql)) 
                    {
                    ...
                    else
                    {
                        ...
                        session_write_close();
                            ...
                            echo '<tr>
                            <th scope="row">1</th><td>'.$bname.'</td><td>'.$author.'</td><td>';
                            echo $today.'</td></tr>';
                            echo '</tbody></table>';
